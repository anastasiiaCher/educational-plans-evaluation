...
def get_bigrams_trigrams(sentences):
    ...
    for sent in sentences:
        # print(collocations.BigramCollocationFinder.from_words(sent))
        # print(collocations.TrigramCollocationFinder.from_words(sent))
        ...
        try:
            phrases = finder_bi.score_ngrams(bigram_measures.likelihood_ratio)
            bigrams_.extend([phrase for phrase, _ in phrases])
        except ValueError:
            pass
        try:
            phrases = finder_tri.score_ngrams(trigram_measures.likelihood_ratio)
            trigrams_.extend([phrase for phrase, _ in phrases])
        except ValueError:
            pass
...
if __name__ == "__main__":
    ...
    # rpd = df.loc[N].text
    rpd = "'Беспроводные сети. В рамках курса студенты изучают сферы применения беспроводных сетей, тенденции развития Bluetooth, WiMAX, 3G/4G/5G, ZigBee, WiFi. Подробнее изучаются протоколы и стандарты передачи данных по технологии IEEE 802.11 a/b/g/n/ac, рассматриваются вопросы построения маршрутов, защиты передачи данных, повышения производительности и отказоустойчивости.. Основные понятия беспроводных сетей (БС). Протоколы семейства IEEE 802.11. Технологии Bluetooth, WiMAX, 4G, ZigBee, WiFi. Безопасность в беспроводных сетях (БС). Области применения БС. Обзор технологий БС. Тенденции развития. Стандарты IEEE 802.11. Обзор технологий WiMAX, 3G/4G/5G. Аутентификация и ассоциация в сетях 802.11. Управление доступом к среде передачи данных. Безопасность передачи данных (WEP/CCMP/WPA2). Методы контроля доступа. Обзор технологии Bluetooth. Обзор технологии ZigBee. Физический уровень стандарта IEEE 802.11'"
    print(rpd)

    start = time()

    print(simple_outcomes_extraction(rpd))
